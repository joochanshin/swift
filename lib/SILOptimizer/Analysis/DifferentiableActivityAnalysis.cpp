...
void DifferentiableActivityInfo::setUsefulThroughArrayInitialization(
  ...
  for (auto use : value->getUses()) {
    ...
    for (auto use : dti->getResult(1)->getUses()) {
      ...
      //   - `apply` with array element address as indirect result.
      ...
      // - `integer_literal` instructions for indexing the `RawPointer`.
      ...
      for (auto use : ptai->getUses()) {
        ...
        if (auto *si = dyn_cast<StoreInst>(user)) {
          ...
          forEachApplyDirectResult(ai, [&](SILValue directResult) {
            setUseful(directResult, dependentVariableIndex);
          });
