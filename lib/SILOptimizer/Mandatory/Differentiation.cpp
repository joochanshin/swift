...
private:
  ...
  LookupConformanceFn getLookupConformanceFunction() {
    ...
    llvm::errs() << "SWIFT MOD: " << getFunction().getModule().getSwiftModule() << "\n";
    llvm::errs() << "ASTCTX: " << &getFunction().getModule().getSwiftModule()->getASTContext() << "\n";
  ...
  /// Marks the given value as varied for and propagates variedness to users.
  ...
  void setDidPropagateVariedness(SILInstruction *inst,
                                 unsigned independentVariableIndex);
  /// Propagates variedness for the given instruction and independent variable
  /// index.
  ...
  /// Marks the given value as varied for the given independent variable index
  /// and recursively propagates variedness inwards (to operands) through
  /// projections. Skips any `@noDerivative` struct field projections.
...
void DifferentiableActivityInfo::propagateVaried(
  ...
  if (didPropagateVariedness(inst, independentVariableIndex))
    return;
  ...
  if (auto *ai = dyn_cast<ApplyInst>(inst)) {
    ...
    for (auto arg : ai->getArgumentsWithoutIndirectResults()) {
      if (isVaried(arg, i)) {
        setDidPropagateVariedness(ai, i);
...
#define PROPAGATE_VARIED_THROUGH_STORE(INST) \
    ...
    if (isVaried(si->getSrc(), i)) {\
      setDidPropagateVariedness(si, i);\
...
#undef PROPAGATE_VARIED_THROUGH_STORE
  // Handle `tuple_element_addr`.
    ...
    if (isVaried(teai->getOperand(), i)) {
      setDidPropagateVariedness(teai, i);
...
#undef PROPAGATE_VARIED_FOR_STRUCT_EXTRACTION
  ...
  // Handle `switch_enum`.
    ...
    if (isVaried(sei->getOperand(), i)) {
      setDidPropagateVariedness(sei, i);
...
void DifferentiableActivityInfo::setDidPropagateVariedness(
    SILInstruction *inst, unsigned independentVariableIndex) {
  ...
  for (auto use : value->getUses()) {
    for (auto res : use->getUser()->getResults()) {
      if (Projection::isAddressProjection(res) || isa<BeginAccessInst>(res)) {
        // Skip non-differentiable projections.
        // if (!res->getType().isDifferentiable(*res->getModule()))
        llvm::errs() << "DERIV GEN SIG: " << derivativeGenericSignature << "\n";
        if (!res->getType().isDifferentiable(getLookupConformanceFunction()))
          continue;
...
bool DifferentiableActivityInfo::didPropagateVariedness(
    SILInstruction *inst, unsigned independentVariableIndex) const {
