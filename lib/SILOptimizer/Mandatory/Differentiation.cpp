...
ADContext::declareExternalAssociatedFunction(
  ...
  llvm::errs() << "CREATING EXTERNAL FUNC NAME: " << name << "\n";
  // module.dump();
  // original->getModule().dump();
...
DifferentiationTask::DifferentiationTask(ADContext &context,
  ...
  llvm::errs() << "CREATING DIFFERENTIATON TASK FOR ORIGINAL " << original->getName() << ", ATTR: " << attr << "\n";
  if (attr->hasJVP()) {
    ...
    if (!jvp)
      jvp = context.declareExternalAssociatedFunction(
          original, attr, attr->getJVPName(), AutoDiffAssociatedFunctionKind::JVP);
  } else {
    if (original->isExternalDeclaration()) {
      ...
      // original->getDeclContext()->getAsDecl()->castTo<FuncDecl>()->g
      // auto decl = dyn_cast<ValueDecl *>(original->getDeclContext()->getAsDecl());
      // auto constant = SILDeclRef(decl, SILDeclRef::Kind::Func);
      // auto jvpName = asAutoDiffAssociatedFunction(
  ...
  if (attr->hasVJP()) {
    ...
    if (!vjp)
      vjp = context.declareExternalAssociatedFunction(
          original, attr, attr->getVJPName(), AutoDiffAssociatedFunctionKind::VJP);
  } else {
    if (original->isExternalDeclaration()) {
      // original->getDeclContext()->getAsDecl()->castTo<FuncDecl>()->g
      // auto decl = dyn_cast<ValueDecl *>(original->getDeclContext()->getAsDecl());
  ...
  if (vjp) {
    ...
    // llvm::errs() << "SKIP VJP SYNTHESIS, DUMPING MODULE\n";
    // module.dump();
    return;
  }
...
void DifferentiationTask::createVJP() {
  ...
  // auto linkage = getAutoDiffFunctionLinkage(original->getLinkage());
  auto linkage = original->getLinkage();
  llvm::errs() << "CREATING VJP FOR ORIGINAL: " << original->getName() << ", ORIGINAL LINKAGE: " << (unsigned)linkage << ", VJP: " << vjpName << "\n";
