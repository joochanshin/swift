...
private:
  ...
  SmallVector<SmallDenseSet<SILValue>, 4> propagatedVariedValueSets;
  ...
  void propagateVaried(SILValue value, unsigned independentVariableIndex);
...
#undef SKIP_NODERIVATIVE
  ...
  for (auto use : value->getUses()) {
    ...
    for (auto &op : use->getUser()->getAllOperands())
      if (isVaried(op.get(), independentVariableIndex))
        for (auto result : inst->getResults())
          setVaried(result, independentVariableIndex);
  }
