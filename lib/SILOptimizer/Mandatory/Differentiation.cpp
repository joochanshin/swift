...
public:
  ...
  bool run() {
    ...
    DenseMap<SILBasicBlock *, SmallVector<std::pair<SILValue, SILValue>, 4>> bufferMapTmp;
    for (auto pair : bufferMap) {
      auto origPair = pair.first;
      auto *bb = origPair.first;
      auto origVal = origPair.second;
      auto adjBuf = pair.second;
      bufferMapTmp[bb].push_back({origVal, adjBuf});
    }
    for (auto pair : bufferMapTmp) {
      auto *bb = pair.first;
      auto valPairs = pair.second;
      llvm::errs() << "BUFFER MAP FOR BB" << bb->getDebugID() << "\n";
      for (auto valPair : valPairs) {
        auto origVal = valPair.first;
        auto adjBuf = valPair.second;
  ...
  void visitSILBasicBlock(SILBasicBlock *bb) {
    ...
    for (auto *bbArg : bb->getArguments()) {
      ...
      for (auto pair : incomingValues) {
        ...
        // TODO: Replace with a `copy_addr`?
        // setAdjointBuffer(predBB, incomingValue, bbArgAdj);
        llvm::errs() << "HOLY COW! ACTIVE BB ARG HAS AN INCOMING VALUE: " << bb->getDebugID() << " -> " << predBB->getDebugID() << "\n";
        bbArg->dump();
        incomingValue->dump();
  ...
  void visitStructExtractInst(StructExtractInst *sei) {
    ...
    // builder.emitDestroyAddrAndFold(cai->getLoc(), tmp);

#if 0
    auto av = getAdjointValue(bb, sei);
    assert(false && "no struct!");
    switch (av.getKind()) {
    case AdjointValueKind::Zero:
...
#if 0
    getOriginal().dump();
    getOriginal().getLocation().dump(getContext().getASTContext().SourceMgr);
    llvm_unreachable("Unhandled: tuple");
