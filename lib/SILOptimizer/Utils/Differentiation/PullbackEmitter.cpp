...
#if 0
  accumulateArrayLiteralElementAddressAdjoints(origBB, originalValue, insertion.first->getSecond(),
...
void PullbackEmitter::accumulateArrayLiteralElementAddressAdjoints(
  ...
  // element addresses: `pointer_to_address` and `index_addr_inst` instructions.
  LLVM_DEBUG(getADDebugStream()
             << "Accumulating array adjoint value into element address adjoint buffers"
             << originalValue);
  llvm::errs() << "ORIG BB: " << origBB->getDebugID() << "\n";
  auto arrayAdjoint = materializeAdjointDirect(arrayAdjointValue, loc);
  llvm::errs() << "ARRAY ADJOINT:\n";
  arrayAdjoint->dump();
...
bool PullbackEmitter::run() {
  ...
  while (auto *bb = domOrder.getNext()) {
    ...
    auto markValueActivity = [&](SILValue v) {
      ...
      if (Projection::isAddressProjection(v) && !isa<IndexAddrInst>(v))
        return;
