...
static ValueDecl *deriveDifferentiable_method(
  ...
  if (!retNominalInit) {
    ...
    retNominalInit = createImplicitConstructor(
        TC, retNominal, ImplicitConstructorKind::Memberwise);

    SmallVector<VarDecl *, 8> diffProperties;
    auto diffPropIt = diffProperties.begin();
    getStoredPropertiesForDifferentiation(retNominal, parentDC, diffProperties);

    TC.resolveDeclSignature(retNominalInit);
    auto
    AutoDiffParameterIndicesBuilder builder(initType);
    for (auto &param : *retNominalInit->getParameters()) {
  ...
  if (nominal == retNominal &&
      !nominal->getAttrs().hasAttribute<FieldwiseDifferentiableAttr>() &&
      retNominalInit->getParent() == parentDC) {
    // TODO: Add this logic for deriving any requirement (allDiffVars and assoc types)
    nominal->getAttrs().add(new (C) FieldwiseDifferentiableAttr(/*implicit*/ true));
  }
  parentDC->dumpContext();
  auto *initMethodType =
      retNominalInit->getMethodInterfaceType()->castTo<AnyFunctionType>();
  unsigned diffParamCount = initMethodType->getNumParams();
  if (retNominalInit->getEffectiveAccess() > AccessLevel::Internal &&
      !retNominalInit->getAttrs().hasAttribute<DifferentiableAttr>() &&
      retNominalInit->getParent() == parentDC &&
      diffParamCount > 0) {
    llvm::errs() << "FINAL STRETCH HERE FOR RET NOMINAL " << retNominal->getName() << "\n";
...
getOrSynthesizeSingleAssociatedStruct(DerivedConformance &derived,
  ...
  if (initDecl->getEffectiveAccess() > AccessLevel::Internal &&
      !initDecl->getAttrs().hasAttribute<DifferentiableAttr>() &&
...
deriveDifferentiable_AssociatedStruct(DerivedConformance &derived,
  ...
  if (initDecl && initDecl->getEffectiveAccess() > AccessLevel::Internal &&
      !initDecl->getAttrs().hasAttribute<DifferentiableAttr>() &&
      !diffProperties.empty()) {
