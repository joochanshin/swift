...
void AttributeChecker::visitDifferentiableAttr(DifferentiableAttr *attr) {
  ...
  (void)evaluateOrDefault(
      ...
      DifferentiableAttributeParameterIndicesRequest{
          attr, attr->getOriginalDeclaration(), /*emitDiagnostics*/ true},
      nullptr);
}

llvm::Expected<IndexSubset *>
DifferentiableAttributeParameterIndicesRequest::evaluate(
    Evaluator &evaluator, DifferentiableAttr *attr, Decl *D,
    bool emitDiagnostics) const {
  llvm::errs() << "ParameterIndicesRequest::evaluate: " << attr << ", D: " << D << ", " << cast<ValueDecl>(D)->getFullName() << "\n";
  if (cast<ValueDecl>(D)->getFullName().getBaseName().userFacingName() == "subscript") {
    llvm::errs() << "FOUND SUBSCRIPT\n";
  }
#if 0
  attr->print(llvm::errs(), D); llvm::errs() << "\n";
#endif
  auto doIfEmitDiagnostics = [&](llvm::function_ref<void(void)> fn) {
    if (!emitDiagnostics)
      return;
    fn();
  };
  ...
  if (attr->isLinear() && (attr->getVJP() || attr->getJVP())) {
    if (emitDiagnostics) {
      diagnoseAndRemoveAttr(
          diags, D, attr, diag::attr_differentiable_no_vjp_or_jvp_when_linear);
      attr->setInvalid();
    }
    return nullptr;
  }
  ...
  if (attr->getJVP()) {
    ...
    FuncDecl *jvp = resolveAutoDiffDerivativeFunction(
        attr->getJVP().getValue(), original, expectedJVPFnTy, isValidJVP, emitDiagnostics);
  ...
  if (attr->getVJP()) {
    ...
    FuncDecl *vjp = resolveAutoDiffDerivativeFunction(
        attr->getVJP().getValue(), original, expectedVJPFnTy, isValidVJP, emitDiagnostics);
