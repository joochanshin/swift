...
#endif
      diagnoseAndRemoveAttr(
          diags, D, attr, diag::differentiable_attr_protocol_req_where_clause);
      return nullptr;
    }
    if (whereClause->getRequirements().empty()) {
      // Where clause must not be empty.
...
#endif
      diagnoseAndRemoveAttr(
          diags, D, attr, diag::differentiable_attr_empty_where_clause);
      return nullptr;
    }
...
#endif
      diagnoseAndRemoveAttr(
          diags, D, attr, diag::differentiable_attr_nongeneric_trailing_where,
          original->getFullName());
      return nullptr;
    }
