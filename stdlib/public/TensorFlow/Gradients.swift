...
func _vjpSquared<T : Differentiable & FloatingPoint>(
  _ x: Tensor<T>
) -> (Tensor<T>, (Tensor<T>) -> Tensor<T>) {
  return (x * x, { v in 2 * x * v })
}
...
extension Tensor where Scalar : Differentiable & FloatingPoint {
  @inlinable
  static func _vjpMatmulOperator(lhs: Tensor, rhs: Tensor)
      -> (Tensor, (Tensor) -> (Tensor, Tensor)) {
  ...
  func _vjpTransposed(
    ...
    return (value, { v in
      let v = v.broadcast(like: value)
      return v.transposed(withPermutations: permutations)
    })
  }
}
