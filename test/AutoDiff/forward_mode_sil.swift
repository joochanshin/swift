...
// CHECK-SIL-LABEL: sil hidden [ossa] @$s5unaryTZp0r0 : $@convention(thin) (Float) -> (Float, @owned @callee_guaranteed (Float) -> Float) {
...
// CHECK-SIL:   [[MULT_FUNC_JVP_1:%.*]] = function_ref @$sSf1moiyS2f_SftFZTZp0_1r0 : $@convention(method) (Float, Float, @thin Float.Type) -> (Float, @owned @callee_guaranteed (Float, Float) -> Float)
// CHECK-SIL:   [[MULT_FUNC_VJP_1:%.*]] = function_ref @$sSf1moiyS2f_SftFZTUp0_1r0 : $@convention(method) (Float, Float, @thin Float.Type) -> (Float, @owned @callee_guaranteed (Float) -> (Float, Float))
// CHECK-SIL:   [[DIFF_INST_1:%.*]] = differentiable_function [wrt 0 1] [order 1] [[MULT_FUNC_1]] : $@convention(method) (Float, Float, @thin Float.Type) -> Float with {[[MULT_FUNC_JVP_1]] : $@convention(method) (Float, Float, @thin Float.Type) -> (Float, @owned @callee_guaranteed (Float, Float) -> Float), [[MULT_FUNC_VJP_1]] : $@convention(method) (Float, Float, @thin Float.Type) -> (Float, @owned @callee_guaranteed (Float) -> (Float, Float))}
// CHECK-SIL:   [[DIFF_EXTRACT_INST_1:%.*]] = differentiable_function_extract [jvp] [order 1] [[DIFF_INST_1]] : $@differentiable @convention(method) (Float, Float, @nondiff @thin Float.Type) -> Float
// CHECK-SIL:   [[MULT_JVP_APPLY_TUPLE_1:%.*]] = apply [[DIFF_EXTRACT_INST_1]]([[X_ARG]], [[X_ARG]], %3) : $@convention(method) (Float, Float, @thin Float.Type) -> (Float, @owned @callee_guaranteed (Float, Float) -> Float)
...
// CHECK-SIL:   [[MULT_JVP_APPLY_TUPLE_2:%.*]] = apply [[AUTODIFF_EXTRACT_INST_1]]([[ORIG_RESULT_1]], [[X_ARG]], %2) : $@convention(method) (Float, Float, @thin Float.Type) -> (Float, @owned @callee_guaranteed (Float, Float) -> Float)
=======
// CHECK-SIL:   [[MULT_FUNC_JVP_2:%.*]] = function_ref @$sSf1moiyS2f_SftFZTZp0_1r0 : $@convention(method) (Float, Float, @thin Float.Type) -> (Float, @owned @callee_guaranteed (Float, Float) -> Float)
// CHECK-SIL:   [[MULT_FUNC_VJP_2:%.*]] = function_ref @$sSf1moiyS2f_SftFZTUp0_1r0 : $@convention(method) (Float, Float, @thin Float.Type) -> (Float, @owned @callee_guaranteed (Float) -> (Float, Float))
// CHECK-SIL:   [[DIFF_INST_2:%.*]] = differentiable_function [wrt 0 1] [order 1] [[MULT_FUNC_2]] : $@convention(method) (Float, Float, @thin Float.Type) -> Float with {[[MULT_FUNC_JVP_2]] : $@convention(method) (Float, Float, @thin Float.Type) -> (Float, @owned @callee_guaranteed (Float, Float) -> Float), [[MULT_FUNC_VJP_2]] : $@convention(method) (Float, Float, @thin Float.Type) -> (Float, @owned @callee_guaranteed (Float) -> (Float, Float))}
// CHECK-SIL:   [[DIFF_EXTRACT_INST_1:%.*]] = differentiable_function_extract [jvp] [order 1] [[DIFF_INST_2]] : $@differentiable @convention(method) (Float, Float, @nondiff @thin Float.Type) -> Float
// CHECK-SIL:   [[MULT_JVP_APPLY_TUPLE_2:%.*]] = apply [[DIFF_EXTRACT_INST_1]]([[ORIG_RESULT_1]], [[X_ARG]], %2) : $@convention(method) (Float, Float, @thin Float.Type) -> (Float, @owned @callee_guaranteed (Float, Float) -> Float)
>>>>>>> Revert "Revert "[AutoDiff] Robust mangling support for AD associated functions. (#26624)" (#26656)"
...
// CHECK-SIL:   [[UNARY_DIFFERENTIAL:%.*]] = function_ref @$s5unaryTzp0r0 : $@convention(thin) (Float, @owned _AD__unary_bb0__DF__src_0_wrt_0) -> Float
...
// CHECK-SIL-LABEL: sil hidden [ossa] @$s5unaryTzp0r0 : $@convention(thin) (Float, @owned _AD__unary_bb0__DF__src_0_wrt_0) -> Float {
...
// CHECK-SIL-LABEL: sil hidden [ossa] @$s6binaryTZp0_1r0 : $@convention(thin) (Float, Float) -> (Float, @owned @callee_guaranteed (Float, Float) -> Float) {
...
// CHECK-SIL:   [[MULT_JVP_APPLY_TUPLE:%.*]] = apply [[AUTODIFF_EXTRACT_INST]]([[X_ARG]], [[Y_ARG]], %4) : $@convention(method) (Float, Float, @thin Float.Type) -> (Float, @owned @callee_guaranteed (Float, Float) -> Float)
=======
// CHECK-SIL:   [[MULT_FUNC_JVP:%.*]] = function_ref @$sSf1moiyS2f_SftFZTZp0_1r0 : $@convention(method) (Float, Float, @thin Float.Type) -> (Float, @owned @callee_guaranteed (Float, Float) -> Float)
// CHECK-SIL:   [[MULT_FUNC_VJP:%.*]] = function_ref @$sSf1moiyS2f_SftFZTUp0_1r0 : $@convention(method) (Float, Float, @thin Float.Type) -> (Float, @owned @callee_guaranteed (Float) -> (Float, Float))
// CHECK-SIL:   [[DIFF_INST:%.*]] = differentiable_function [wrt 0 1] [order 1] [[MULT_FUNC]] : $@convention(method) (Float, Float, @thin Float.Type) -> Float with {[[MULT_FUNC_JVP]] : $@convention(method) (Float, Float, @thin Float.Type) -> (Float, @owned @callee_guaranteed (Float, Float) -> Float), [[MULT_FUNC_VJP]] : $@convention(method) (Float, Float, @thin Float.Type) -> (Float, @owned @callee_guaranteed (Float) -> (Float, Float))}
// CHECK-SIL:   [[DIFF_EXTRACT_INST:%.*]] = differentiable_function_extract [jvp] [order 1] [[DIFF_INST]] : $@differentiable @convention(method) (Float, Float, @nondiff @thin Float.Type) -> Float
// CHECK-SIL:   [[MULT_JVP_APPLY_TUPLE:%.*]] = apply [[DIFF_EXTRACT_INST]]([[X_ARG]], [[Y_ARG]], %4) : $@convention(method) (Float, Float, @thin Float.Type) -> (Float, @owned @callee_guaranteed (Float, Float) -> Float)
>>>>>>> Revert "Revert "[AutoDiff] Robust mangling support for AD associated functions. (#26624)" (#26656)"
...
// CHECK-SIL:   [[BINARY_DIFFERENTIAL:%.*]] = function_ref @$s6binaryTzp0_1r0 : $@convention(thin) (Float, Float, @owned _AD__binary_bb0__DF__src_0_wrt_0_1) -> Float
...
// CHECK-SIL-LABEL: sil hidden [ossa] @$s6binaryTzp0_1r0 : $@convention(thin) (Float, Float, @owned _AD__binary_bb0__DF__src_0_wrt_0_1) -> Float {
